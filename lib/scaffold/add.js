 ...
 * @param {String} options.dirname - The artbytecore-node configuration directory
...
function add(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'artbytecore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a artbytecore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        // add service to artbytecore-node.json
