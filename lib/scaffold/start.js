...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for bitcoind is necessary for this upgrade with the "reindex=1" applebyte.conf option. \n' +
                 'There are changes necessary in both applebyte.conf and artbytecore-node.json. \n\n' +
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/whitecoind')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './artbytecore-node.json');
