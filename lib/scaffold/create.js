...
var BASE_PACKAGE = {
  ...
  readme: 'README.md',
  dependencies: {
    'artbytecore-lib': '^' + bitcore.version,
    'artbytecore-node': version
  }
};

/**
 * Will create a directory and applebyte.conf file for ArtByte.
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/artbytecore-node.json', configJSON);
      if (!isGlobal) {
 ...
 * @param {String} options.dirname - The name of the artbytecore node configuration directory
 * @param {String} options.datadir - The path to the artbytecoin datadir
